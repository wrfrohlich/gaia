...
def load_data(file_path):
    ...
    df = pd.DataFrame(data, columns=['Frame', 'Time', 'Fx1', 'Fy1', 'Fz1', 'Fx2', 'Fy2', 'Fz2'])
...
def get_gait_symmetry(raw_data):
    ...
    symmetry_indices = {
        'sym_idx_fx': (2 * abs(peaks['Fx1'] - peaks['Fx2']) / (peaks['Fx1'] + peaks['Fx2'])),
        'sym_idx_fy': (2 * abs(peaks['Fy1'] - peaks['Fy2']) / (peaks['Fy1'] + peaks['Fy2'])),
        'sym_idx_fz': (2 * abs(peaks['Fz1'] - peaks['Fz2']) / (peaks['Fz1'] + peaks['Fz2'])),
    }
...
def butter_lowpass_filter(data, cutoff, fs, order=5):
    ...
    isnan = np.isnan(data)
    filtered_data = np.copy(data)  # Criar uma cópia dos dados para preencher com dados filtrados
...
def analyze_file(file_path):
    ...
    single_support_right_list, single_support_left_list, double_support_list = get_support_phase(raw_data, "Fy1", "Fy2")
    ...
    number_of_steps = get_number_of_steps(single_support_right_list, single_support_left_list)
    ...
    step_time_sup = get_step_time_sup(single_support_right_list, single_support_left_list, double_support_list)
