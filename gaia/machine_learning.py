...
class MachineLearning:
    ...
    def run(self, sep_data, data, method:Literal["linear_regression", "random_forest"]="linear_regression"):
        for item in sep_data:
            ...
            report.to_csv(f'{self.path_experiment}/report_{method}.csv', mode='a', header=not path.exists(f'{self.path_experiment}/report_{method}.csv'), index=False)

    def plot_feature_importance(self, item, model, x_train):
        importance = model.feature_importances_
        features = x_train.columns

        sns.barplot(x=importance, y=features)
        plt.title(f"Feature Importance - {item}")
        ...
        report.insert(0, 'item', item)
        report.to_csv(f'{self.path_experiment}/feature_importance.csv', mode='a', header=not path.exists(f'{self.path_experiment}/feature_importance.csv'), index=False)
