...
class Clustering:
    ...
    def run_clustering_kmeans(self, data, method="pca", n_clusters=3, n_components=2):
        ...
        for _, row in correlation_data.iterrows():
            ...
            if shifted_data.shape[0] > 1:
                ...
                if method == "pca":
                    reduced_data = PCA(n_components=n_components).fit_transform(shifted_data)
                elif method == "tsne":
                    reduced_data = TSNE(n_components=2, perplexity=30, n_iter=300, random_state=42).fit_transform(shifted_data)

                # Clustering
                clusters = KMeans(n_clusters=n_clusters, random_state=42).fit_predict(reduced_data)
    ...
    def analyze_cross_correlation(self):
        ...
        for kinematic in important_points:
            filtered_df = data[data["kinematic"] == kinematic].sort_values(by=["imu"]).reset_index(drop=True)
            if not filtered_df.empty:
                correlation_report[kinematic] = filtered_df["imu"].tolist()

        if correlation_report:
            df_report = pd.DataFrame.from_dict(correlation_report, orient='index').fillna("NaN")
